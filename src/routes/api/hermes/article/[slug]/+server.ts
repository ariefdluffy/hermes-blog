...
function authenticateRequest(request: Request): { ok: true; scope: ApiKeyScope } | { ok: false; code: ErrorCode; status: number; reason: string } {
	...
	if (!hash) {
		return { ok: false, code: 'INTERNAL_ERROR', status: 503, reason: 'Hermes agent not configured' };
	}
...
// GET /api/hermes/article/[slug] — get single article by slug
...
// PATCH /api/hermes/article/[id] — update article (status, content, etc.)
